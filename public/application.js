...
/***/ function(module, exports, __webpack_require__) {
	...
	var onGetGame = function onGetGame(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.getGame(data.game.id).then(ui.getGameSuccess).catch(ui.getGameFailure);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var updateGame = __webpack_require__(10).updateGame;
	...
	var boardInteraction = function boardInteraction() {
	  $('#auth-message').text('');
	  var update = { cell: { index: $(this).data('id'), value: turn }, over: win };
	  if (this.textContent === '') {
	    $(this).text(turn);
	...
	var changeMessage = function changeMessage(txt) {
	  var element = $('#message')[0];
	  $(element).text(txt);
	};
	...
	var checkDraw = function checkDraw() {
	  var result = gameArray.some(function (element) {
	    return element === '';
	  });
	  if (result === false) {
	    win = true;
